...
class StackOverFlowAnalysis():
    def __init__(self, input_file):
        ...
        self.data['continent'] = self.data.apply(lambda x: pc.country_alpha2_to_continent_code(
            pc.country_name_to_country_alpha2(coco.convert(x['Country']), cn_name_format="default")), axis=1)
    ...
    def pythonDeveForCountry(self, country='All'):
        try:
            ...
            if country == 'All':
                final = \
                self.data[self.data['LanguageWorkedWith'].str.contains('Python')].groupby("Country").agg('count')[
                    'Respondent'] / total * 100
            else:
                final = self.data[
                            self.data['LanguageWorkedWith'].str.contains('Python') & self.data['Country'].str.contains(
                                country)].groupby("Country").agg('count')['Respondent'] / total * 100
            print(final)
        except Exception as e:
            print("Exception/Error occured:{}".format(e))
    ...
    def avgSalOfDeve(self, continent='All'):
        try:
            if continent == 'All':
                ...
                res = self.data[self.data['continent'] == continent].fillna(0).groupby('continent')[
                    'ConvertedComp'].mean()
            print(res)
    ...
    def hobbyAsCode(self, continent="All"):
        ...
        try:
            if continent == 'All':
                code = self.data[self.data["Hobbyist"] == 'Yes'].groupby(['continent', 'Gender']).agg('count')[
                    'Respondent']
            else:
                code = self.data[self.data["Hobbyist"] == 'Yes'] & \
                       self.data[self.data['continent'] == 'continent'].groupby(['continent', 'Gender']).agg('count')[
                           'Respondent']
            print(code)
        except Exception as e:
            print("Exception/Error occured:{}".format(e))
...
if __name__ == '__main__':
    a = StackOverFlowAnalysis("survey_results_public.csv")
